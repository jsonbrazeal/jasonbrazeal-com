...
# Get directory containing Vagrantfile to name the VM the same
VAGRANTDIR = File.basename(Dir.getwd)
PROJECTDIR = File.basename(File.expand_path(".",Dir.pwd))
PROJECTPATH = File.expand_path(".")
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # Provision VM using this shell script, which runs 'fab vagrant provision'
  ...
  config.vm.synced_folder PROJECTPATH, "/opt/" + PROJECTDIR #, owner: "apache", group: "apache"
  # owner and group commented out above because they only work if apache is installed

  # Provider-specific configuration so you can fine-tune various
  # backing providers for Vagrant. These expose provider-specific options.

  config.vm.define (PROJECTDIR +  ' vm') do |vm|
  end

  config.vm.provider :virtualbox do |vb|
      vb.name = PROJECTDIR +  ' vm'

      ##################PyPE settings##################
      # This forces VirtualBox to use the host's DNS resolver instead of
      # VirtualBox's, which sucks
